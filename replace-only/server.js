...
app.post("/api/bookings/general-guidance", async (req, res) => {
  try {
    const { name, email, phone, serviceCategory, preferredDate, preferredTime, queryDescription } = req.body;
    let booking;
    if (useDB) {
      const count = await Booking.countDocuments();
      booking = await Booking.create({
        bookingRef: makeRef("BK", count + 1), name, email, phone,
        serviceCategory: serviceCategory || "General Guidance", preferredDate: preferredDate||"",
      ...
      booking = { id: store.nextIds.booking, bookingRef: makeRef("BK", store.nextIds.booking), name, email, phone, serviceCategory: serviceCategory||"General Guidance", preferredDate:"", preferredTime:"", consultantName:"", queryDescription: queryDescription||"", consultationType:"generalGuidance", status:"pending", paymentStatus:"pending", contactedStatus:"new", adminNotes:"", createdAt: Date.now() };
      store.nextIds.booking++; store.bookings.push(booking); saveData();
    }
...
// ── PUBLIC: mark payment as failed (no admin auth — called when customer cancels Razorpay) ──
app.post("/api/bookings/:id/mark-failed", async (req, res) => {
  try {
    const { id } = req.params;
    if (useDB) {
      await Booking.findByIdAndUpdate(id, { paymentStatus: "failed" });
    } else {
      const b = store.bookings.find(b => String(b.id) === String(id));
      if (b) { b.paymentStatus = "failed"; saveData(); }
    }
    res.json({ ok: true });
  } catch (err) {
    res.status(500).json({ error: err.message });
