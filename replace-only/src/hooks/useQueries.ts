...
// Admin-only fetch — redirects to /admin login on 401/403
...
// Public fetch — never redirects to /admin (used for payment, bookings, contact)
...
export function useSubmitBooking() {
  ...
  return useMutation({
    mutationFn: async (data: BookingFormData) => {
      return publicFetch("/bookings", { method: "POST", body: JSON.stringify({
...
export function useSubmitGeneralGuidanceBooking() {
  ...
  return useMutation({
    mutationFn: async (data: BookingFormData) => {
      return publicFetch("/bookings/general-guidance", { method: "POST", body: JSON.stringify({
        name: data.name, email: data.email, phone: data.phone,
...
// No pagination — returns all bookings (admin only)
...
// Public: mark a booking payment as failed (no admin auth needed)
export function useMarkPaymentFailed() {
  return useMutation({
    mutationFn: async (bookingId: string) => {
      return publicFetch(`/bookings/${bookingId}/mark-failed`, { method: "POST" });
    },
  });
}
...
export function useGetPaymentConfig() {
  return useQuery({ queryKey: ["payment-config"], queryFn: () => publicFetch("/payment/config") });
}
export function useCreateRazorpayOrder() {
  return useMutation({
    mutationFn: async (data: { bookingId: any; amount?: number }) =>
      publicFetch("/payment/create-order", { method: "POST", body: JSON.stringify(data) }),
  });
}
export function useVerifyRazorpayPayment() {
  return useMutation({
    mutationFn: async (data: { razorpay_order_id: string; razorpay_payment_id: string; razorpay_signature: string; bookingId: any }) =>
      publicFetch("/payment/verify", { method: "POST", body: JSON.stringify(data) }),
  });
}
...
export function useSubmitContactLead() {
  ...
  return useMutation({
    mutationFn: async (data: ContactFormData) =>
      publicFetch("/contact-leads", { method: "POST", body: JSON.stringify(data) }),
...
export function useGetTestimonials() {
  return useQuery<Testimonial[]>({ queryKey: ["testimonials"], queryFn: () => publicFetch("/testimonials") });
}
...
export function useSubmitTestimonial() {
  ...
  return useMutation({
    mutationFn: async (data: TestimonialFormData) =>
      publicFetch("/testimonials", { method: "POST", body: JSON.stringify({
...
export function useSubscribeNewsletter() {
  return useMutation({
    mutationFn: async (data: { email: string; name?: string }) =>
      publicFetch("/newsletter/subscribe", { method: "POST", body: JSON.stringify(data) }),
  });
}
