...
import { useCreateRazorpayOrder, useVerifyRazorpayPayment, useGetPaymentConfig, useUpdateBooking } from "@/hooks/useQueries";
import {
  AlertCircle,
  CheckCircle2,
  IndianRupee,
...
function getParams() {
  ...
  return {
    // MongoDB returns string _id, numeric fallback for local json store
...
export function PaymentPage() {
  ...
  const [bookingId, setBookingId] = useState<string | null>(params.bookingId);
  const isInitiated = useRef(false);

  // If no bookingId in URL params (direct visit), just show form info
  ...
  const handlePay = async () => {
    ...
    try {
      ...
      // If stub mode (no Razorpay keys) — just mark success
      if (order.stub) {
      ...
      const rzp = new (window as any).Razorpay({
        ...
        modal: {
          ondismiss: () => {
            ...
            // Mark booking as payment_failed in DB when user closes Razorpay
            if (bookingId) {
              updateBooking.mutateAsync({ id: bookingId, paymentStatus: "failed" }).catch(() => {});
            }
            setStatus("idle");
          },
        },
      ...
      console.error(err);
      isInitiated.current = false;
      // Mark booking as payment_failed when order creation itself fails
      if (bookingId) {
        updateBooking.mutateAsync({ id: bookingId, paymentStatus: "failed" }).catch(() => {});
      }
      setStatus("failed");
